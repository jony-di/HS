...
namespace HumansoftServer
{
    public class Util
    {
        ...
        public static byte[] converHTMLToPDF(string htmlText,string pathLogo)
        {
            ...
            //addLogo.Border = Rectangle.BOX;
            //addLogo.BorderColor = BaseColor.YELLOW;
            //addLogo.BorderWidth = 5f;
            MemoryStream ms = null;
            ...
            try
            {
                ...
                if (File.Exists(pathLogo))
                {
                    ...
                    addLogo = Image.GetInstance(pathLogo, true);
                    addLogo.ScaleToFit(128, 37);
                    addLogo.Alignment = iTextSharp.text.Image.ALIGN_RIGHT;
                    //addLogo.SetAbsolutePosition(50, pdfDocument.PageSize.Height - 50);
                    pdfDocument.Add(addLogo);
                }
            ...
            public static DataSet GetArchivos(string carpeta,string buscar="*.*",bool subCarpetas=false)
        {
            ...
            public static uint ObtenerIndiceSiguienteArchivoDia(string ruta) {
                string IDENTITY = @"HKEY_LOCAL_MACHINE\Software\HSoft\" + ruta;
                const string DATE = "Dia";
                const string COUNT = "CUENTA";
                string dia = (string)Registry.GetValue(IDENTITY, DATE, "Dia");
                string aux = (string)Registry.GetValue(IDENTITY, COUNT, "0"); ;
                uint consecutivo = string.IsNullOrEmpty(aux) ? 0 : uint.Parse(aux, CultureInfo.CurrentCulture);
                if (string.IsNullOrEmpty(dia) || !string.Equals(dia, DateTime.Now.ToString("ddMMyy", CultureInfo.CurrentCulture)))
                {
                    dia = DateTime.Now.ToString("ddMMyy", CultureInfo.CurrentCulture);
                    Registry.SetValue(IDENTITY, DATE, dia);
                    consecutivo = 1;
                    Registry.SetValue(IDENTITY, COUNT, consecutivo);
                }
                else
                {
                    ++consecutivo;
