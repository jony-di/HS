...
function obtenerEtiquetaValores() {

    $.post(urlBase_WS + "NCatalogoGenerico.aspx", "seccion=Elemento&op=obtenerEtiquetasValor&Catalogoclave=" + CLAVE_CatalogoGenerico).done(function (xmlDoc) {
        var Tabla = xmlDoc.getElementsByTagName("Table");
        ValorPrincipal = GetValor(Tabla[0], "label1");
        ValorSecundario = GetValor(Tabla[0], "label2");
        if (ValorPrincipal !== "") {
            document.getElementById("Label1").innerHTML = ValorPrincipal;
            document.getElementById("Label11").innerHTML = ValorPrincipal;
        }
        if (ValorSecundario !== "") {
            document.getElementById("Label2").innerHTML = ValorSecundario;
            document.getElementById("Label22").innerHTML = ValorSecundario;
        }
        cargarCatalogoCatalogoGenerico();
    });
}
...
function buscarCoincidencias(ev, objeto) {
    ...
    cargarCatalogoCatalogoGenerico(1, criterio);
}
...
var ordenador;
function cargarCatalogoCatalogoGenerico(pagina, criterio) {
    PonerEnEspera();
    var paginador = document.getElementById("paginador");
    if (pagina == undefined) {
        pagina = paginador.paginaActual == undefined ? 1 : paginador.paginaActual;
    }
    $.post(urlBase_WS + "NCatalogoGenerico.aspx", { seccion: "Elementos", op: "obtenerElementos", pagina: pagina, longitudPagina: 5, criterio: (criterio ? criterio : ""), cve_catalogo: CLAVE_CatalogoGenerico }).done(function (xmlDoc) {
        QuitarEspera();
...
function CargarDatosCatalogoGenerico(CatalogoGenerico) {
    $.post(urlBase_WS + "NCatalogoGenerico.aspx", { seccion: "Elemento", op: "obtenerElemento", clave: CatalogoGenerico, cve_catalogo: CLAVE_CatalogoGenerico, pagina: 1, longitudPagina: 5
        ...
        SetValor(CatalogoG, "cve_subcatalogo", "clave");
        SetValor(CatalogoG, "descripcion", "descripcion");
        SetValor(CatalogoG, "valor1", "Valor1");
        SetValor(CatalogoG, "valor2", "Valor2");
        SetValor(CatalogoG, "activo", "estatus", "bool");
    });
}
