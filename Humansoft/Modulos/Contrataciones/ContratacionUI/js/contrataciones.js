...
window.onresize = function () {
    ...
    for (i = 0; i < scrollables.length; i++) {
        var offset=parseInt(scrollables[i].getAttribute("offset"), 10) || 310;
        scrollables[i].style.height = (alturaWindow - offset) + "px";
    }
}
...
function AgregarCriterio(placeholder, criterio, idPadre, tipo, url, leyenda, descripcion, clave) {
    ...
    if (numeroFiltros < 6) {
        if (!tipo || tipo === "input") {
            NuevoElemento(document.getElementById(idPadre), "span", "criterio-busqueda", "<input name='" + criterio + "' placeholder='" + placeholder + "' onkeyup=\"ActualizarPosiciones();\"/><button class='btnFormularios' onclick='EliminarCriterio(this,ActualizarPosiciones);'>x</button>", "");
        } else if (tipo === "select") {
...
function AgregarCriterioEmpleado(placeholder, criterio, idPadre, tipo, url, leyenda, descripcion, clave) {
    ...
    if (numeroFiltros < 6) {
        if (!tipo || tipo === "input") {
            NuevoElemento(document.getElementById(idPadre), "span", "criterio-busqueda", "<input name='" + criterio + "' placeholder='" + placeholder + "' onkeyup=\"cargarCatalogoEmpleado(1,'');\"/><button class='btnFormularios' onclick='EliminarCriterio(this,ConsultarEmpleados)'>x</button>", "");
        } else if (tipo === "select") {
            LlenarSelect(url, undefined, leyenda, clave, descripcion,'', function (selectRelleno) {
                if (selectRelleno.options[0].value === '0') {
                    selectRelleno.options[0].value = "";
...
function LlenarSelect(url, idSelect, leyenda, clave, descripcion, attach, callback) {
    var selectUI;
    if (idSelect) {
    ...
    $.post(url).done(function (xmlDoc) {
        ...
        for (var i = 0; i < departamento.length; i++) {
            ...
            optionitem.setAttribute("attached", GetValor(departamento[i], attach));
            selectUI.appendChild(optionitem);
        }
        if (callback != undefined) {
            callback(selectUI, departamento);
        }
    });
}
...
function LlenarCatalogoPaises(selectUI, callback) {
    ...
    optionitem.value = "0";
    selectUI.appendChild(optionitem);
    $.post("/Modulos/Seguridad/SeguridadNegocio/" + "NPaises.aspx", { op: "obtenerCatalogoPais", pagina: 1, longitudPagina: 1000, criterio: "",
        cve_pais: 0
    }).done(function (xmlDoc) {
...
function LlenarCatalogoEmpresa(selectUI, callback) {
    ...
    optionitem.value = "0";
    selectUI.appendChild(optionitem);
    $.post("/Modulos/Seguridad/SeguridadNegocio/" + "NEmpresas.aspx", { op: "obtenerCatalogoEmpresas", pagina: 1, longitudPagina: 1000, criterio: "",
        cve_empresa: 0
    }).done(function (xmlDoc) {
...
function LlenarCatalogoMunicipio(selectUI, callback) {
    selectUI.innerHTML = "";
    var optionitem = document.createElement("option");
...
function LlenarCatalogoSegmento(selectUI, callback) {
    ...
    $.post(urlBase_WS + "NSegmento.aspx", { op: "obtenerCatalogoSegmentos", pagina: 1, longitudPagina: 1000, criterio: "",
        ...
        for (var i = 0; i < segmento.length; i++) {
            ...
            optionitem.value = GetValor(segmento[i], "segmento");
            selectUI.appendChild(optionitem);
        }
        if (callback != undefined) {
            callback();
