...
function cargarCatalogoItemCat(pagina, criterio, cve_catalogo) {
    ...
    $.post(urlBase_WS + "NCatalogosGenericos.aspx", { seccion: "CatalogosGenericos", op: "obtenerCatalogos", pagina: pagina, longitudPagina: 5, criterio: (criterio ? criterio : ""), cve_catalogo: cve_catalogo }).done(function (xmlDoc) {
        ...
        for (var i = 0; i < ItemCat.length; i++) {
            ...
            var Etiqueta1 = GetValor(ItemCat[i], "etiqueta1");
            var Etiqueta2 = GetValor(ItemCat[i], "etiqueta2");
            ...
            $(itemLista).html(
                ...
                //'<td><label class="etiqueta">' + Etiqueta1 + '</label></td>' +
                //'<td><label class="etiqueta">' + Etiqueta1 + '</label></td>' +
...
function VerDetalleCatalogoGenerico(cveCatalogo, row) {
    ...
    window.frames["pantallaAuxiliar"].location.href = "/Modulos/Estructuras/EstructurasUI/ES_CatalogoGenerico.aspx?callbackInicio=callbackInicioVerDetalleCatalogoGenerico&offset=0&CatalogoGenerico=" + cveCatalogo;
}
