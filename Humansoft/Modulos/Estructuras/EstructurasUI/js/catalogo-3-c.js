...
function crearNuevo() {
    ...
    $.post(urlBase_WS + "NCatalogosTresCampos.aspx", "op=Nuevo&seccion=TabuladorDescripciones&" + parametros).done(function (xmlDoc) {
...
function MostrarNuevo() {
    ...
    $.post(urlBase_WS + "NCatalogosTresCampos.aspx", { seccion: "TabuladorDescripciones", op: "obtenerSiguienteClave" }).done(function (xmlDoc) {
...
var ordenador;
function cargarCatalogoItemCat(pagina, criterio, cve_tabulador) {
    if (!cve_tabulador) cve_tabulador = 0;
    PonerEnEspera();
    var paginador = document.getElementById("paginador");
    if (pagina == undefined) {
        pagina = paginador.paginaActual == undefined ? 1 : paginador.paginaActual;
    }
    $.post(urlBase_WS + "NCatalogosTresCampos.aspx", { seccion: "TabuladorDescripciones", op: "obtenerCatalogo", pagina: pagina, longitudPagina: 5, criterio: (criterio ? criterio : ""), cve_tabulador: cve_tabulador }).done(function (xmlDoc) {
        QuitarEspera();
        ...
        for (var i = 0; i < ItemCat.length; i++) {
            var CveItemCat = GetValor(ItemCat[i], "cve_tabulador");
            ...
            $(itemLista).html(
                ...
                '<td><label class="seleccionar"><img class="seleccionar" onclick="VerDetalleTabulador(' + CveItemCat + ',this)" src="/Recursos/imagenes/select.png"/></label></td>'
            );
...
function VerDetalleTabulador(cveTabulador, row) {
    PonerEnEspera();
    IntercambioVisual("pantallaAuxiliar","formulario");
    window.frames["pantallaAuxiliar"].location.href = "/Modulos/Estructuras/EstructurasUI/ES_Tabulador.aspx?callbackInicio=callbackInicioVerDetalleTabulador&offset=0&tabulador=" + cveTabulador;
}

function callbackInicioVerDetalleTabulador() {
    QuitarEspera();
...
function Guardar() {
    if (confirm("¿Esta seguro que desea guardar los cambios?")) {
        ...
        if (!frmItemCat.esEditar) {
            ...
            GuardarEdicioNCatalogosTresCampos();
        }
    } else {
        MostrarCatalogo();
    }
...
function CargarDatosItemCat(cve_ItemCat) {
    $.post(urlBase_WS + "NCatalogosTresCampos.aspx", { seccion: "TabuladorDescripciones", op: "obtenerCatalogo", pagina: 1, longitudPagina: 5, criterio: "",
        cve_tabulador: cve_ItemCat
    }).done(function (xmlDoc) {
        var ItemCat = xmlDoc.getElementsByTagName("Table")[0];
        SetValor(ItemCat, "cve_tabulador", "clave");
        SetValor(ItemCat, "descripcion", "descripcion");
        SetValor(ItemCat, "activo", "estatus", "bool");
    });
}

function GuardarEdicioNCatalogosTresCampos() {
    var frmNuevo = document.getElementById("frmNuevo");
    var parametros = $(frmNuevo).serialize();
    $.post(urlBase_WS + "NCatalogosTresCampos.aspx", "op=Editar&seccion=TabuladorDescripciones&" + parametros).done(function (xmlDoc) {
        var estatus = GetValor(xmlDoc, "estatus");
        var mensaje = GetValor(xmlDoc, "mensaje");
        var notificacion = document.getElementById("notificacion");
        var mensajeNotificacion = document.getElementById("mensaje-alerta");
...
function DesactivarItemCat(cve_departamento) {
    $.post(urlBase_WS + "NCatalogosTresCampos.aspx", { op: "CambiarEstatusActivo", cve_departamento: cve_departamento, activo: false }).done(function (xmlDoc) {
        //  alert(xmlDoc);
        cargarCatalogoItemCat();
    })
}
