<%@ Page Language="C#"%>
<%@ Import Namespace="System.Data"%>
<%@ Import Namespace="HumansoftServer"%>
<script runat="server">
    
    protected void Page_Load(object sender, EventArgs e)
    {
        Response.Clear();
        Response.ContentType = "text/xml";
        switch (Request["op"])
        {
            case "NuevoPaqConcepto": Nuevo(); break;
            case "CambiarEstatusActivo": CambiarEstatusActivo(); break;
            case "EditarPaqConcepto": Editar(); break;
            case "obtenerCatalogoPaqConcepto": ObtenerCatalogo(); break;
            case "obtenerSiguienteClave": ObtenerSiguienteClave(); break;
        }
    }

    public void ObtenerSiguienteClave()
    {
        DataSet ds = new Modelo().GenerarOperacion(
            new object[0, 0], "PA_ES_CObtenerSiguienteClavePaqueteConceptos"
        );
        ds.WriteXml(Response.OutputStream);
    }

    public void Nuevo()
    {
        DataSet ds = new Modelo().GenerarOperacion(
            new object[,] { 
                   { "Clave de concepto", "cve_concepto", Request["cve_concepto"],true,"int", 0}
                ,  { "Clave de dado", "cve_dado", Request["cve_dado"],true,"int", 0}
                ,  { "Clave de tipo", "cve_tipo", Request["cve_tipo"],true,"int", 0}
                ,  { "Descripción", "descripcion", Request["descripcion"],true,"string", 150}
                ,  { "Calculo", "caculo", Request["calculo"],true,"string", 50}
                ,  { "Activo", "activo","true", true ,"bool", 0}
            }
            , "PA_ES_INuevoPaqueteConceptos"
        );
        ds.WriteXml(Response.OutputStream);
    }

    public void CambiarEstatusActivo()
    {
        DataSet ds = new Modelo().GenerarOperacion(
            new object[,] { 
                  { "Clave de concepto", "cve_concepto", Request["cve_concepto"],true,"int", 0}
                , { "Estatus", "activo",Request["activo"], true, "bool", 0 }
            }
            , "PA_ES_UCambiarEstatusActivoPaqueteConceptos"
        );
        ds.WriteXml(Response.OutputStream);
    }

    public void Editar()
    {
        DataSet ds = new Modelo().GenerarOperacion(
            new object[,] { 
                   { "Clave de concepto", "cve_concepto", Request["cve_concepto"],true,"int", 0}
                ,  { "Clave de dado", "cve_dado", Request["cve_dado"],true,"int", 0}
                ,  { "Clave de tipo", "cve_tipo", Request["cve_tipo"],true,"int", 0}
                ,  { "Descripción", "descripcion", Request["descripcion"],true,"string", 150}
                ,  { "Calculo", "caculo", Request["calculo"],true,"string", 50}
                ,  { "Activo", "activo",Request["activo"], true ,"bool", 0}
            }
            , "PA_ES_UActualizarPaqueteConceptos"
        );
        ds.WriteXml(Response.OutputStream);
    }

    public void ObtenerCatalogo()
    {
        DataSet ds = new Modelo().GenerarOperacion(
            new object[,] { 
                  { "Número de página", "pagina", Request["pagina"],true,"int", 0}
                , { "Longitud de página", "longitudPagina",Request["longitudPagina"], true ,"string", 80}
                , { "Criterio de búsqueda", "criterio",Request["criterio"], false ,"string", 80}
                , { "Clave de concepto", "cve_concepto", Request["cve_concepto"],true,"int", 0}
            }
            , "PA_ES_CBusquedaPaqueteConceptos"
        );
        ds.WriteXml(Response.OutputStream);
    }
    
</script>
